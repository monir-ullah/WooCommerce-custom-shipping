                ...
                <?php foreach ($items as $row): ?>
                    <tr>
                        <td><input type="text" class="cz-label" value="<?php echo esc_attr($row['label']); ?>"
                                placeholder="e.g. Inside Dhaka" style="width:100%"></td>
                        <td><input type="number" step="0.01" min="0" class="cz-price"
                                value="<?php echo esc_attr($row['price']); ?>" placeholder="0.00" style="width:100%"></td>
...
// -----------------------------
// HIDE WooCommerce default shipping when custom zones enabled
// -----------------------------
add_filter('woocommerce_cart_ready_to_calc_shipping', function ($show_shipping) {
    ...
    if ($enabled === 'yes' && is_array($zones) && !empty($zones)) {
        return false; // Hide default shipping
    }
...
// -----------------------------
// Add custom city zones as inline radio buttons
// -----------------------------
add_action('woocommerce_review_order_before_payment', function () {
    ...
    ?>
    <tr class="wc-city-zones-row">
        <th>Shipping:</th>
        <td>
            <div id="wc-city-zones" style="padding:10px 0;">
                <?php foreach ($zones as $index => $zone):
                    ...
                    ?>
                    <label style="display:inline-block;margin-right:20px;margin-bottom:8px;cursor:pointer;">
                        <input type="radio" name="wc_city_zone" value="<?php echo $index; ?>"
                            data-cost="<?php echo $zone_price; ?>" <?php echo $checked; ?> style="margin-right:5px;">
                        <strong><?php echo $zone_label; ?></strong> - <?php echo wc_price($zone_price); ?>
                    </label>
                <?php endforeach; ?>
            </div>
        </td>
...
add_action('woocommerce_checkout_update_order_review', function ($posted_data) {
    ...
    // Check if custom zones are enabled
    $enabled = get_option('wc_city_zones_enabled', 'no');
    if ($enabled !== 'yes')
        return;
    ...
    if (isset($output['wc_city_zone'])) {
        ...
        // Set default to first zone if not selected
        WC()->session->set('wc_city_zone', '0');
    }
});
...
add_action('woocommerce_cart_calculate_fees', function () {
    ...
    // Default to first zone if nothing selected
    if ($selected_zone === null || $selected_zone === '') {
        $selected_zone = '0';
        WC()->session->set('wc_city_zone', '0');
    }
    ...
    if (isset($zones[$index]['price'])) {
        $cost = floatval($zones[$index]['price']);
        $label = !empty($zones[$index]['label']) ? $zones[$index]['label'] : 'Shipping';
        WC()->cart->add_fee($label, $cost);
    }
}, 20);

// -----------------------------
// AJAX update on zone selection
// -----------------------------
add_action('wp_footer', function () {
    ...
    <script type="text/javascript">
        jQuery(function ($) {
            // Trigger update when radio button changes
            $(document).on('change', 'input[name="wc_city_zone"]', function () {
                $('body').trigger('update_checkout');
            });

            // Ensure first option is checked on page load
            if ($('input[name="wc_city_zone"]:checked').length === 0) {
                $('input[name="wc_city_zone"]:first').prop('checked', true).trigger('change');
            }
        });
    </script>
    <style>
        /* Hide WooCommerce default shipping when custom zones active */
...
// -----------------------------
// Hide default shipping row completely
// -----------------------------
add_filter('woocommerce_cart_shipping_packages', function ($packages) {
    ...
    if ($enabled === 'yes' && is_array($zones) && !empty($zones)) {
        return []; // Return empty to hide default shipping
    }
