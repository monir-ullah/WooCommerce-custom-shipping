...
// -----------------------------
// Display shipping zone in order summary (NEW UI)
// -----------------------------
add_action('woocommerce_review_order_after_order_total', function () {
    ...
    <tr class="wc-city-zones-display">
        <th><?php echo esc_html($selected_label); ?>:</th>
        <td data-title="<?php echo esc_attr($selected_label); ?>">
            <div id="wc-city-zones-selector" style="margin:0;">
                ...
                ?>
                    <label style="display:inline-block;margin-right:15px;margin-bottom:5px;cursor:pointer;font-size:14px;">
                        <input type="radio" 
                               name="wc_city_zone" 
                        ...
                        <span style="vertical-align:middle;"><?php echo $zone_label; ?> - <strong><?php echo wc_price($zone_price); ?></strong></span>
                    </label>
                <?php endforeach; ?>
            </div>
        </td>
...
add_action('wp_footer', function () {
    ...
    <script type="text/javascript">
        jQuery(function ($) {
            // Update the label when radio changes
            $(document).on('change', 'input.wc-city-zone-radio', function () {
                var selectedLabel = $(this).data('label');
                $('.wc-city-zones-display th').text(selectedLabel + ':');
                $('body').trigger('update_checkout');
            });
            
            // Ensure first option is checked on page load
            if ($('input.wc-city-zone-radio:checked').length === 0) {
                $('input.wc-city-zone-radio:first').prop('checked', true);
                var firstLabel = $('input.wc-city-zone-radio:first').data('label');
                $('.wc-city-zones-display th').text(firstLabel + ':');
            }
        });
    </script>
    <style>
        ...
        /* Style for city zones in order summary */
